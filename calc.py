...
def calc(data,b,A_Magn,DEF2ATK):

    calc_hit = {buff.on:{},buff.off:{}}#每刀伤害
    calc_calc_ATK = {buff.on:0,buff.off:0}#开大攻击力
    calc_expect = {buff.on:0,buff.off:0}#伤害期望
    
    for st in buff.status_list:
        calc_DEF = data["DEF"]+b.get(st,"DEF")*data["basic_DEF"]
        calc_ATK = data["ATK"]+b.get(st,"ATK")*data["basic_ATK"]+calc_DEF*DEF2ATK
        calc_calc_ATK[st] = calc_ATK
    ...
    return calc_hit,calc_calc_ATK,calc_expect
